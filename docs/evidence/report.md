...
- **Old Ceiling (Highest Passing Stage)**: **400 Locust users**.
  - Dựa trên ảnh snapshot, ghi nhận **76.2 RPS (current/snapshot RPS)**, với tỷ lệ lỗi 0%. Do không có CSV exact-window chứng minh, các số liệu này được ghi nhận là snapshot thay vì sustained 5 phút.
- **Breakpoint (Failing Stage)**: **410 Locust users**.
  - Dựa trên ảnh snapshot, ghi nhận **73.9 RPS (current/snapshot RPS)**, với tỷ lệ lỗi 1% failures.
- **Requests-per-node Baseline**: 76.2 RPS / 9 nodes = 8.46 RPS/node (tính theo snapshot RPS).

## 2. Stage comparison
| Stage | Traffic mix | Current/Snapshot RPS | Failures | SLO status |
|---|---|---:|---:|---|
| Highest passing 400 users | Browse 70% / Cart 20% / Checkout 10% | 76.2 | 0% | Pass |
| Failing 410 users | Browse 70% / Cart 20% / Checkout 10% | 73.9 | 1% | Fail: breach SLO |
