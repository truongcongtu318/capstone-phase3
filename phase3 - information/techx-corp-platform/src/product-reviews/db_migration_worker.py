...
def migrate_is_safe_column(batch_size=500, sleep_between=0.1):
    ...
    try:
        ...
        # migration.sql uses CREATE INDEX CONCURRENTLY, which PostgreSQL refuses to
        # run inside a transaction block. psycopg2 opens one implicitly, so the
        # schema step must run with autocommit ON; it is turned back off afterwards
        # so the batch scan below keeps its explicit per-batch commits.
