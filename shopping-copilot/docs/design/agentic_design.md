...
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  CopilotAgent (Amazon Bedrock - Nova Lite)                     │  │
│  │                                                                │  │
│  │  ┌─────────┐   ┌──────────┐                                    │  │
│  │  │ L1      │ → │ L2a/L2b  │                                    │  │
│  │  │ Rate    │   │ Input    │                                    │  │
...
│  │                     │                                          │  │
│  │         ┌───────────▼───────────┐                              │  │
│  │         │ 1. Intent Parser (LLM)│ (Phân tích Ý định)           │  │
│  │         └───────────┬───────────┘                              │  │
│  │                     │ Intent JSON (task_type, products...)     │  │
│  │         ┌───────────▼───────────┐                              │  │
│  │         │ 2. Planner (Python)   │ (Lập kế hoạch động)          │  │
│  │         └───────────┬───────────┘                              │  │
│  │                     │ Execution Plan (list of tools)           │  │
│  │         ┌───────────▼───────────┐   (Guardrails L3, L4 chèn    │  │
│  │         │ 3. Executor (Python)  │   vào trước mỗi tool call)   │  │
│  │         └───────────┬───────────┘                              │  │
│  │                     │ Raw JSON results from gRPC/REST          │  │
│  │         ┌───────────▼───────────┐                              │  │
│  │         │ 4. Evidence Aggregator│ (Gom nhóm Bằng chứng)        │  │
│  │         └───────────┬───────────┘                              │  │
│  │                     │ Evidence JSON + __intent_meta__          │  │
│  │         ┌───────────▼───────────┐                              │  │
│  │         │ 5/6. Answer Generator │ (Sinh phản hồi an toàn, đa   │  │
│  │         │      & Grounding (LLM)│ ngôn ngữ dựa trên Evidence)  │  │
│  │         └───────────┬───────────┘                              │  │
│  │                     │ Text Response                            │  │
│  │  ┌──────────────────▼──────────────────┐                       │  │
...
## 6. Agent Core — Kiến trúc 6 Lớp (Intent-Driven)

**File:** `agent/copilot_agent.py` và `llm/prompt.py`.

Thay vì sử dụng ReAct loop truyền thống (LLM tự do gọi tool vòng lặp) vốn dễ gặp lỗi hallucination và infinite loop, hệ thống triển khai kiến trúc 6 lớp định hướng theo ý định, đảm bảo tính deterministic (dự đoán được) và khả năng hỗ trợ đa ngôn ngữ tuyệt đối (Zero Hard-Rules).

### 6.1 Lớp 1: Intent Parser (Phân Tích Ý Định)
Mục tiêu là biên dịch câu nói tự nhiên (bằng bất kỳ ngôn ngữ nào) thành một `Intent Schema` JSON.
- **LLM Prompt:** `INTENT_PARSE_PROMPT`
- **Chức năng:** Trích xuất `task_type` (add_to_cart, search, rank, lookup, etc.), tham số tìm kiếm (`price_max`, `currency`), và đặc biệt là phân giải đại từ chỉ định (`context_reference` như "cái này", "cái thứ hai") dựa trên `last_search_results` trong Context (đếm 1-based index).
- **Quy tắc phân biệt Rank vs Search:** Nếu người dùng hỏi "cái nào rẻ hơn" (trong số các sản phẩm đang hiển thị), gán `rank`. Nếu hỏi "còn cái nào rẻ hơn không" (tìm ngoài), gán `search`.

### 6.2 Lớp 2: Generic Planner (Lập Kế Hoạch Động)
- **Công nghệ:** Code Python (Không dùng LLM).
- **Chức năng:** Nhận `Intent` JSON và ánh xạ thành một `Execution Plan` (danh sách các Tool cần gọi tuần tự).
- **Tính lắp ghép (Modular):** Nếu Intent chứa `needs_reviews=True`, Planner sẽ linh hoạt đính kèm (append) tool `__fetch_reviews_for_context__` vào cuối kế hoạch mà không cần tạo rule cứng.

### 6.3 Lớp 3: Executor (Thực Thi)
- Thực thi kế hoạch tuần tự. Mỗi tool call sẽ tự động đi qua **Lớp 3 Tool Validator** (Allow-list, User Isolation) và **Lớp 4 Confirmation Gate** (HMAC token cho Write Actions).
- Tự động thay thế biến phụ thuộc (dependency resolution), ví dụ: `$PREV` sẽ được tự động đổi thành `last_product_id` từ kết quả của tool trước đó hoặc từ Context.

### 6.4 Lớp 4: Evidence Aggregator (Gom Bằng Chứng)
- Thu thập toàn bộ JSON trả về từ các tool thành một đối tượng `evidence` duy nhất.
- Cập nhật Context (`last_product_id`, `last_search_ids`) vào Memory (`SessionStore`) để chuẩn bị cho các câu hỏi theo ngữ cảnh tiếp theo.
- Nhúng siêu dữ liệu vào Evidence: `evidence["__intent_meta__"] = {"task_type": "...", ...}` để LLM hiểu được hành động ban đầu.

### 6.5 Lớp 5 & 6: Answer Generator & Grounding
- **LLM Prompt:** `EVIDENCE_SYNTHESIS_PROMPT`
- LLM nhận `Evidence JSON` + `Câu hỏi của người dùng`.
- **Zero Hard-Rules:** Mọi phản hồi (từ chối thao tác, xin chào, hay so sánh sản phẩm) đều do LLM sinh ra. Điều này giúp hệ thống tự động phát hiện ngôn ngữ của người dùng và trả lời bằng ngôn ngữ đó mà không cần bất kỳ câu lệnh `if/else` ngôn ngữ cứng nào.
- **Grounding (Chống Hallucination):** LLM bị cấm tuyệt đối việc bịa ra thông tin. Nó chỉ được dùng dữ liệu trong `Evidence`. Nếu Evidence rỗng, nó phải báo rõ.

### 6.6 Hướng dẫn chạy Agent
Hệ thống sử dụng mô hình **Amazon Nova Lite** qua **AWS Bedrock**. Cấu hình trong `.env`:
```env
